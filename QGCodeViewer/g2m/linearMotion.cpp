...
linearMotion::linearMotion(std::string canonL, machineStatus prevStatus): canonMotion(canonL,prevStatus) 
{
    ...
    //  startDir = (status.getStartPose().dir + status.getOrigin().dir) * (SIGN * M_PI/180.0);
    startDir.x = (status.getStartPose().dir.x + status.getOrigin().dir.x) * (SIGN_A * M_PI/180.0);
    startDir.y = (status.getStartPose().dir.y + status.getOrigin().dir.y) * (SIGN_B * M_PI/180.0);
    startDir.z = (status.getStartPose().dir.z + status.getOrigin().dir.z) * (SIGN_C * M_PI/180.0);
    //  endDir = (status.getEndPose().dir + status.getOrigin().dir) * (SIGN * M_PI/180.0);
    endDir.x = (status.getEndPose().dir.x + status.getOrigin().dir.x) * (SIGN_A * M_PI/180.0);
    endDir.y = (status.getEndPose().dir.y + status.getOrigin().dir.y) * (SIGN_B * M_PI/180.0);
    endDir.z = (status.getEndPose().dir.z + status.getOrigin().dir.z) * (SIGN_C * M_PI/180.0);
#endif
}
...
Point linearMotion::point(double s) 
{
    if ( length() == 0.0 )
        ...
        // std::cerr << "linearMotion::point() ERROR at s= " << s << " length= " << length() << " t evaluates to t= " << t << "\n";
