...
{
    debug = false; 
    initialPos = Pose( Point(0,0,0), Point(0,0,0) ); 
    userOrigin  = Pose( Point(0,0,0), Point(0,0,0) ); total_gcode_lines = 0; 
}
...
std::vector<canonLine*> g2m::getCanonLines() 
{ 
    return lineVector; 
}

void g2m::setInitialPos(Pose init) 
{ 
    initialPos = init;
}

void g2m::setOrigin(Pose origin) 
{
    userOrigin = origin; 
}
...
void g2m::setFile(QString infile) 
{
...
void g2m::setToolTable(QString tbl_file) 
{ 
...
void g2m::setInterp(QString interp_binary) 
{ 
...
void g2m::interpret_file() 
{
    ...
    if ( file.endsWith(".ngc") ) 
        ...
        while(std::getline(inFile, sLine)) 
            {
            if (sLine.length() > 1) 
                {  //helps to prevent segfault in canonLine::cmdMatch()
...
bool g2m::processCanonLine(std::string l) 
{
    ...
    else   // use the last element status
